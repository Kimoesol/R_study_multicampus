...
국어 <- c(4,7,6,8,5,5,9,10,4,10)
par(mar=c(1,1,1,1), mfrow=c(4,2)) # 여러 개의 그래프를 동시 출력
# mar: 상하좌우 margin / mfrow는 반드시 두 개의 아규먼트를 주어야 한다
...
# x, y 축 추가하기
axis(1, at=1:10, lab=c("01","02","03","04", "05","06","07","08","09","10")) # x축 추가(첫 번째 아규먼트가 1)
axis(2, at=c(0,2,4,6,8,10))  # y축 추가(첫 번째 아규먼트가 2)
...
dev.off() # 이 함수를 꼭 실행해야 png 파일이 온전히 만들어짐

plot(국어,수학)
plot(수학~국어) # 국어 성적에 기반한 수학 성적 분포는 어떻게 되는가?
...
barplot(as.matrix(성적1), main="성적그래프", ylab="점수", col=rainbow(10)) # 디폴트: 스텝 방식
barplot(t(성적1), main="성적그래프", ylab="점수", col=rainbow(10)) # 학생 별 점수
# t(): transform 함수[행과 열을 바꿔준다]
...
# 파이그래프 [디폴트: (1)시작점: 3시 방향, (2)반시계]
(성적 <- read.table("성적.txt", header=TRUE));
pie(성적$국어, labels=paste(성적$성명, "-", 성적$국어), col=rainbow(10))
pie(성적$국어, clockwise=T, labels=paste(성적$성명, "-", 성적$국어), col=rainbow(10)) # 시계방향일 때는 시작점: 12시 방향
...
hist(성적$수학, main="성적분포-수학", xlab="점수", col = "lightblue", breaks=2, border = "pink")
...
library(xlsx)
classDF <- read.xlsx("data.xlsx", 1, encoding="UTF-8") # 1: sheet1을 의미
str(classDF)
View(classDF)
